...
class InvalidSymbolTracker:
        ...
        if os.path.exists(self.filename):
            try:
                with open(self.filename, 'r') as f:
                    ...
                    print(f"📚 Loaded {len(self.invalid_symbols)} invalid symbols")
            except Exception as e:
                print(f"⚠️ Error loading invalid symbols: {e}")
                self.invalid_symbols = {}
        else:
            print("📝 Creating new invalid symbols tracker")
            self.save()
    ...
    def cleanup_old_entries(self, days=90):
        ...
        if removed:
            print(f"🧹 Removed {len(removed)} old invalid symbols")
            self.save()
    ...
    def retry_symbol(self, symbol):
        ...
        if symbol in self.invalid_symbols:
            del self.invalid_symbols[symbol]
            self.save()
            print(f"♻️ Removed {symbol} from invalid list for retry")
            return True
        return False