"""
🌙 Moon Dev RBI Strategy Validator
...
try:
    ...
    cprint("✅ backtesting.py library available", "green")
except ImportError:
    BACKTESTING_AVAILABLE = False
    cprint("⚠️ backtesting.py not available - using simplified validation", "yellow")
...
class StrategyAnalyzer:
    ...
    def extract_strategy_info(file_path: str) -> Dict:
        ...
        try:
            ...
            cprint(f"❌ Error analyzing {file_path}: {str(e)}", "red")
    ...
    def validate_strategy_syntax(file_path: str) -> bool:
        ...
        try:
            ...
            cprint(f"❌ Error validating {file_path}: {e}", "red")
            return False
        ...
        if data_files:
            try:
                ...
                if all(col in df.columns for col in required_cols):
                    ...
                    cprint(f"✅ Loaded {len(df)} rows of {symbol} data", "green")
                    return df
            except Exception as e:
                cprint(f"❌ Error loading data from {data_files[0]}: {e}", "red")
        
        # Generate synthetic data if no real data available
        cprint(f"⚠️ No data file found for {symbol}, generating synthetic data", "yellow")
    ...
    def run_backtest(self, strategy_file: str, data: pd.DataFrame, 
        ...
        try:
            ...
            if not strategy_classes:
                cprint(f"❌ No strategy class found in {strategy_file}", "red")
            ...
            cprint(f"❌ Error running backtest for {strategy_file}: {str(e)}", "red")
            traceback.print_exc()
            return {"error": str(e)}
...
class RBIStrategyValidator(BaseAgent):
    ...
    def discover_rbi_strategies(self) -> List[str]:
        ...
        cprint(f"📁 Found {len(strategy_files)} strategy files from RBI", "green")
        ...
        for file_path in strategy_files:
            if self.analyzer.validate_strategy_syntax(file_path):
                ...
                cprint(f"  ✅ {Path(file_path).name}", "white")
            else:
                cprint(f"  ❌ {Path(file_path).name} (syntax error)", "red")
    ...
    def validate_strategy(self, strategy_file: str) -> Optional[StrategyMetrics]:
        ...
        cprint(f"\n🧪 Validating strategy: {Path(strategy_file).name}", "cyan")
        
        # Analyze strategy structure
        strategy_info = self.analyzer.extract_strategy_info(strategy_file)
        
        if "error" in strategy_info:
            cprint(f"❌ Analysis failed: {strategy_info['error']}", "red")
            return None
        
        cprint(f"📋 Found {len(strategy_info.get('strategy_classes', []))} strategy classes", "white")
        cprint(f"📊 Using {len(strategy_info.get('indicators_used', []))} indicators", "white")
        ...
        for timeframe in self.validation_config["timeframes"]:
            for period in self.validation_config["test_periods"]:
                cprint(f"  🔄 Testing {timeframe} timeframe, {period} days...", "white")
                
                # Load market data
                data = self.backtest_runner.load_market_data("BTC", timeframe, period)
                
                if data is None or len(data) < 20:
                    cprint(f"  ⚠️ Insufficient data for {timeframe}/{period}d", "yellow")
                    continue
                ...
                if "error" not in result:
                    ...
                    cprint(f"    📈 Return: {return_pct:+.1f}%, Sharpe: {sharpe:.2f}", 
                           "green" if return_pct > 0 else "red")
        
        if not all_results:
            cprint("❌ No successful backtests", "red")
            return None
        ...
        if self._meets_validation_criteria(aggregate_metrics):
            cprint(f"✅ Strategy validation PASSED", "green")
            
            # Save to database
            self.db.save_strategy_metrics(aggregate_metrics)
            return aggregate_metrics
        else:
            cprint(f"❌ Strategy validation FAILED", "red")
            return None
    ...
    def _meets_validation_criteria(self, metrics: StrategyMetrics) -> bool:
        ...
        for check_name, passed in checks:
            if passed:
                cprint(f"  ✅ {check_name}", "green")
                passed_checks += 1
            else:
                cprint(f"  ❌ {check_name}", "red")
    ...
    def run_validation_pipeline(self) -> List[StrategyMetrics]:
        ...
        cprint("\n🌙 Moon Dev RBI Strategy Validation Pipeline", "cyan", attrs=["bold"])
        cprint("=" * 60, "blue")
        ...
        try:
            ...
            if not strategy_files:
                cprint("⚠️ No strategies found from RBI agent", "yellow")
                cprint("💡 Run the RBI agent first to generate strategies", "white")
                return []
            ...
            cprint(f"\n✅ Validation Pipeline Complete!", "green", attrs=["bold"])
            cprint(f"🔍 Tested: {len(strategy_files)} strategies", "green")
            cprint(f"✅ Validated: {len(validated_strategies)} profitable strategies", "green")
            
            if validated_strategies:
                best_strategy = max(validated_strategies, key=lambda x: x.sharpe_ratio)
                cprint(f"🏆 Best Strategy: {best_strategy.strategy_id} (Sharpe: {best_strategy.sharpe_ratio:.2f})", "green")
            
            return validated_strategies
            
        except Exception as e:
            cprint(f"❌ Error in validation pipeline: {str(e)}", "red")
            return []
    ...
    def _generate_validation_report(self, strategies: List[StrategyMetrics]):
        ...
        cprint(f"💾 Validation report saved: {report_file}", "green")
...
def main():
    ...
    if validated_strategies:
        ...
        cprint(f"\n💡 No strategies validated. Try:", "cyan")
