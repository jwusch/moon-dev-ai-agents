...
class ForceBandContraction(Strategy):
    ...
    def calculate_position_size(self, long=True):
        """Calculate proper position size based on risk percentage"""
        risk_amount = self.equity * self.risk_percent
        stop_distance = 2 * self.atr[-1]
        
        if long:
            stop_price = self.data.Close[-1] - stop_distance
            size = risk_amount / (self.data.Close[-1] - stop_price)
        else:
            stop_price = self.data.Close[-1] + stop_distance
            size = risk_amount / (stop_price - self.data.Close[-1])
        
        # Ensure position size is valid
        size = min(size, 1.0)  # Never risk more than 100% of equity
        return size
        ...
        if not self.position and atr_valid:
            if efi_cross and bw_contraction:
                ...
                if self.data.Close[-1] > self.bb_upper[-1]:
                    size = self.calculate_position_size(long=True)