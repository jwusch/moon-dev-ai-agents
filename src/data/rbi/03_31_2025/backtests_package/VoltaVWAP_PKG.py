...
#             if price[-2] < self.upper_band[-2] and price[-1] > self.upper_band[-1]:
#                 self.enter_trade('long')
#             # 🌙 Short entry condition (bearish crossover)    
#             elif self.lower_band[-2] > price[-2] and self.lower_band[-1] < price[-1]:
...
#             if self.vwap[-2] > price[-2] and self.vwap[-1] < price[-1]:
#                 self.position.close()
#                 print(f"🎯🌙 LUNAR PROFIT HARVEST | Closed at {price:.2f} | Moon Phase: Waxing")
