"""
Moon Dev's LiquidityMomentum Strategy
...
#             if (crossover(self.cmo[-3:], self.upper_band[-3:]) and
...
#             elif (crossunder(self.cmo[-3:], self.lower_band[-3:]) and
#                 any(abs(price - lvl) < price*0.01 for lvl in fib_levels.values()) and
#                 abs(price - self.swing_high[-1]) < price*0.01):
#                 
#                 risk_amount = self.equity * self.risk_pct
#                 sl = self.swing_high[-