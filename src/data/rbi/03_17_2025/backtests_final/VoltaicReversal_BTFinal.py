"""
Moon Dev's VoltaicReversal Strategy
...
# Here's the debugged and fixed version of your backtest code with Moon Dev themed implementations and proper position sizing rules:
# 
# ```python
# # -*- coding: utf-8 -*-
# import pandas as pd
...
#                         print(f"🌙✨🚀 LONG SIGNAL! Buying {size} units @ {current_close:.2f}")
...
#                     if size > 0:  # Moon Dev safety check ✨
#                         tp_price = current_close - 1.5 * current_atr
#                         print(f"🌙✨🚀 SHORT SIGNAL! Selling {size} units @ {current_close:.2f}")
#                         self.sell(size=size, sl=sl_price, tp=tp_price)
# 
#         # Moon Dev trend reversal exits 🌗
#         if self.position.is_long and (self.data.Close[-2] > self.ema50[-2] and self.data.Close[-1] < self.ema50[-1]):
#             print(f"🌙✨ Closing LONG @ {current_close:.2f} (EMA50 Reversal)")
#             self.position.close()
#         elif self.position.is_short and (self.data.Close[-2] < self.ema50[-2] and self.data.Close[-1] > self.ema50[-1]):
