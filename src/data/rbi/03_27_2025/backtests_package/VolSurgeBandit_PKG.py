"""
Moon Dev's VolSurgeBandit Strategy
...
# 
#         # 💎 Exit Logic
#         if self.position.is_long and (self.vwap[-2] < self.data.Close[-2] and self.vwap[-1] > self.data.Close[-1]):
#             self.position.close()
#             print(f"🌙📉 LONG EXIT at VWAP {self.vwap[-1]:.2f}")
#             
#         if self.position.is_short and (self.data.Close[-2] < self.vwap[-2] and self.data.Close