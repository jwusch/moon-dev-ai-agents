    ...
    def next(self):
        ...
        if not self.position:
            ...
            if bb_contraction and vol_spike and price_breakout:
                ...
                atr_value = max(self.atr[-1], 0.01)  # Ensure minimum ATR value to prevent division by zero
...
#                     print(f"🌙 MOON DEV ALERT: Long Entry at {self.data.Close[-1]:.2f} | Size: {position_size} units | ATR Stop: {self.data.Close[-1] - atr_value:.2f} 🚀")
        else:
            ...
            if self.data.Low[-1] < self.trail_stop:
                self.position.close()
#                 print(f"🌙 MOON DEV EXIT: Trailing Stop Triggered at {self.trail_stop:.2f}"