...
class DynamicBandSqueeze(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(lambda: talib.BBANDS(self.data.Close, self.bb_period, 2, 2)[0])
        self.bb_middle = self.I(lambda: talib.BBANDS(self.data.Close, self.bb_period, 2, 2)[1])
        self.bb_lower = self.I(lambda: talib.BBANDS(self.data.Close, self.bb_period, 2, 2)[2])
        
        # Keltner Channel Magic 🌙
        self.keltner_mid = self.I(talib.EMA, self.data.Close, self.bb_period)
        self.atr = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, self.bb_period)
