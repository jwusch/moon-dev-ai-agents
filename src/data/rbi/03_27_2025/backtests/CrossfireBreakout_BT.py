"""
Moon Dev's CrossfireBreakout Strategy
...
#     
#     def init(self):
#         # Clean data and handle columns already done in data preparation
...
#         ema50_cross_under = crossover(self.ema200, self.ema50)
...
#             print(f"🌙🚀 LONG ENTRY | Price: {self.data.Close[-1]:.2f} | Size: {position_size} | SL: {self.stop_loss:.2f}")
# 
#     def enter_short(self):
#         risk_amount = self.equity * self.risk_per_trade
#         atr_value = self.atr[-1]
#         stop_loss_price = self.data.Close[-1] + 2 * atr_value
#         risk_per_share = stop_loss_price - self.data.Close[-1]
#         
#         if risk_per_share <= 0:
#             return
#             
