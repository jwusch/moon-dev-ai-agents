"""
Moon Dev's VortexVolMACD Strategy
...
#             print("🌙 Moon Dev Warning: Not enough data for reliable signals")
#             return
# 
#         # Get current values
...
#         vi_bearish = (self.vi_plus[-2] > self.vi_minus[-2]) and (self.vi_plus[-1] < self.vi_minus[-1])
#         macd_bullish = (self.macd_line[-2] < self.signal_line[-2]) and (self.macd_line[-1] > self.signal_line[-1])
#         macd_bearish = (self.macd_line[-2] > self.signal_line[-2]) and (self.macd_line[-1] < self.signal_line[-1])
