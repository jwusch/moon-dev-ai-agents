"""
Moon Dev's DivergencePulse Strategy
...
#         # print(f"🌙 Current Close: {self.data.Close[-1]:0.2f} | EMA5: {self.ema5[-1]:0.2f} | EMA20: {self.ema20[-1]:0.2f} | ATR: {self.atr[-1]:0.2f}")
#         
#         # Manage existing positions
#         if self.position.is_long:
#             # Update trailing stop for longs
#             new_swing_low = self.swing_low[-1]
#             self.trailing_stop = max(self.trailing_stop, new_swing_low)
#             
#             if self.data.Low[-1] <= self.trailing_stop:
#                 self.position.close()
#                 print(f"🌙✨ Trailing Stop Hit! Closing Long at {self.data.Close[-1]:0.2f}")
#                 
#         elif self.position.is_short:
#             # Update trailing stop for shorts
#             new_swing_high = self.swing_high[-1]
#             self.trailing_stop = min(self.trailing_stop, new_swing_high)
#             
#             if self.data.High[-1] >= self.trailing_stop:
#                 self.position.close()
#                 print(f"🌙✨ Trailing Stop Hit! Closing Short at {self.data.Close[-1]:0.2f}")
#         
#         # Entry logic when no position exists
#         if not self.position:
#             # Bullish divergence with volume spike
#             if crossover(self.ema5, self.ema20) and \
...
#                     print(f"🚀🌕 BULLISH DIVERGENCE DETECTED! Long {position_size} units at {entry_price:0.2f}")
#             
#             # Bearish divergence with volume spike
#             elif crossover(self.ema20, self.ema5) and \
#                  self.data.Volume[-1] > self.volume_mult