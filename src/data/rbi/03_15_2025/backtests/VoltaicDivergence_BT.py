...
class VoltaicDivergence(Strategy):
    ...
    def init(self):
        ...
        # Swing detection
        self.max_high = self.I(talib.MAX, self.data.High, self.swing_window)
        self.min_low = self.I(talib.MIN, self.data.Low, self.swing_window)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Long entry
            if current_close > self.upper_band[-1] and bull_div:
                risk_amount = self.equity[-1] * self.risk_pct / 100
                atr_val = self.atr[-1]
                sl_price = current_close - 1.5*atr_val
