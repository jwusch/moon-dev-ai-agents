"""
Moon Dev's DivergenceSqueeze Strategy
...
#         print(f"🌙 Current Price: {price:0.2f} | MACD: {self.macd[-1]:0.2f} | Volume: {self.vol[-1]:0.2f}")
#         
...
#             print(f"🚀🌕 MOON DEV LONG ENTRY! Size: {position_size} | Entry: {price:0.2f} | SL: {stop_loss:0.2f}")
#     
...
#             print(f"🌑🌙 MOON DEV SHORT ENTRY! Size: {position_size} | Entry: {price:0.2f} | SL: {stop_loss:0.2f}")
#     
...
#                 self.position.close()
#                 print(f"✨🎯 Closing LONG at {self.data.Close[-1]:0.2f} (Exit Signal)")
#         
#         elif self.position.is_short:
#             if (self.data.Low[-1] <= self.bb_lower[-1] or
#                 crossover(self.macd, self.macd_signal)):
#                 self.position.close()
#                 print(f"✨🎯 Closing SHORT at {self.data.Close[-1]:0.2f} (Exit Signal)")
# 
# # Data preparation
