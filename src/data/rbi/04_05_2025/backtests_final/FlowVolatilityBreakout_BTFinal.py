"""
Moon Dev's FlowVolatilityBreakout Strategy
...
#         self.atr22 = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, timeperiod=22)
#         
#         # 🌊 Netflow Indicators
#         self.netflow_ma20 = self.I(talib.SMA, self.data.Netflow, timeperiod=20)
#         self.netflow_std = self.I(talib.STDDEV, self.data.Netflow, timeperiod=20)
#         
#         # 🎯 Swing Points
...
# 
#     def _valid_short_conditions(self):
#         bear_div = (self.data.High[-3:].idxmax() == len(self.data)-1 and 
#                    self.data.Netflow[-3:].idxmax() < len(self.data)-1)
#         return (self.adx[-1] > 25 and
#                 self.data.Close[-1] < self.ema200[-1] and
#                 bear_div and
#                 self.data.Close[-1] < self.swing_low[-1] and
#                 (self.netflow_ma20[-1] - self.data.Netflow[-1]) > 2*self.netflow_std[-1])
# 
