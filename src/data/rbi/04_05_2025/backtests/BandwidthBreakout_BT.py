"""
Moon Dev's BandwidthBreakout Strategy
...
#                         print(f"🚀🌙 Moon Dev LONG Signal! Size: {position_size} | Risk: {risk:.2f}")
# 
#                 # Short Entry 🌑
...
#                         self.entry_bar = len(self.data)
#                         print(f"🌑🌙 Moon Dev SHORT Signal! Size: {position_size} | Risk: {risk:.2f}")
# 
#         # 🌙 Exit Logic
#         else:
...
#                     self.position.close()
#                     print(f"🌙✨ Closing LONG: {'Band Touch' if self.data.Low[-1] <= bb_lower else 'Time Exit'}")
# 
#             elif self.position.is_short:
#                 # Stop loss or opposite band touch
#                 if self.data.High[-1] >= bb_upper or bars_since_entry >= 5:
#                     self.position.close()
#                     print(f"🌙✨ Closing SHORT: {'Band Touch' if self.data.High[-1] >= bb_