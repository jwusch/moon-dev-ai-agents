...
class DualMomentumFisher(Strategy):
    ...
    def _check_entries(self):
        ...
        fisher_buy = (self.fisher[-2] < self.signal[-2] and self.fisher[-1] > self.signal[-1])  # Bullish crossover replacement
        ...
        fisher_sell = (self.fisher[-2] > self.signal[-2] and self.fisher[-1] < self.signal[-1])  # Bearish crossover replacement
    ...
    def _enter_trade(self, direction):
        ...
        if direction == 'long':
            stop_loss = entry_price - 2 * current_c