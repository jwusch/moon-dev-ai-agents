"""
Moon Dev's ChaikinBreakout Strategy
...
#                     print(f"🌙🚀 BULLISH BREAKOUT! Long {units} units at {self.data.Close[-1]:.2f} | SL: {stop_price:.2f}")
# 
#             # Short entry
...
#                 if units > 0:
#                     stop_price = self.data.Close[-1] * (1 + self.stop_loss)
#                     self.sell(size=units, sl=stop_price)
#                     print(f"🌙✨ BEARISH BREAKOUT! Short {units} units at {self.data.Close[-1]:.2f} | SL: {stop_price:.2f}")
# 
#         else:
#             # Exit logic
...
# 
# bt = Backtest(data, ChaikinBreakout, cash=1_000_000, commission=.002)
# stats = bt.run()
# print(stats)
