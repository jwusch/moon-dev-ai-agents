...
class VolumetricFisher(Strategy):
    ...
    def next(self):
        ...
        # Bullish crossover replacements
        fisher_bullish = (self.fisher[-2] < self.fisher_signal[-2] and self.fisher[-1] > self.fisher_signal[-1])
        macd_bullish = (self.macd_line[-2] < self.macd_signal[-2] and self.macd_line[-1] > self.macd_signal[-1])
        
        # Bearish crossover replacements
        fisher_bearish = (self.fisher[-2] > self.fisher_signal[-2] and self.fisher[-1] < self.fisher_signal[-1])
        macd_bearish = (self.macd_line[-2] > self.macd_signal[-2] and self.macd_line[-1] < self.macd_signal[-1])
        
        long_entry = (fisher_bullish and 
                     macd_bullish and 
                     fisher_above_upper and macd_above_upper)
        
        short_entry = (fisher_bearish and 
                      macd_bearish and 
...
#                 print(f"🌙 MOON DEV LONG ENTRY 🚀 | Size: {position_size} | Price: {self.data.Close[-1]:.2f} | SL"