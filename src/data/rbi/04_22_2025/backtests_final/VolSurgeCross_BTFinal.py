"""
Moon Dev's VolSurgeCross Strategy
...
#             # Long entry
#             if (hv20[-2] < hv5[-2] and hv20[-1] > hv5[-1]) and current_vol > 1.5*vol_ma:
#                 risk = 0.01 * self.equity
#                 sl_pct = 2 * hv5
#                 size = int(round(risk / (close * sl_pct)))
#                 if size > 0:
#                     self.buy(size=size, sl=close*(1-sl_pct))
#                     print(f"🌙✨ MOON DEV LONG 🚀 | Size: {size} | Entry: {close:.2f} | SL: {close*(1-sl_pct):.2f}")
#             
#             # Short entry
#             elif (hv5[-2] > hv20[-2] and hv5[-1] < hv20[-1]) and current_vol > 1.5*vol_ma:
#                 risk = 0.01 * self.equity
#                 sl_pct = 2 * hv5
#                 size = int(round(risk / (close * sl_pct)))
#                 if size > 0:
#                     self.sell(size=size, sl=close*(1+sl_pct))
#                     print(f"🌙✨ MOON DEV SHORT 💫 | Size: {size} | Entry: {close:.2f} | SL: {close*(1+sl_pct):.2f}")
#         else:
#             if self.hv_3[-1] > self.hv_3_90th[-1]:
#                 self.position.close()
#                 print(f"🌪️ VOL SURGE EXIT | Price: {close:.2f} | Current HV3: {self.hv_3[-1]:.4f} | 90th%: {self.hv_3_90th[-