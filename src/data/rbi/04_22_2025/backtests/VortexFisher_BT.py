...
#         # Short entry logic        
#         if not self.position:
#             if (crossover(vi_minus, vi_plus)) and \
#                (fisher[-1] < 2 and fisher[-2] >= 2) and \
#                (self.data.Volume[-1] > self.data.Volume[-2]):
#                 
...
#         # Exit logic
#         if self.position:
#             if (self.position.is_long and crossover(vi_minus, vi_plus)) or \
#                (self.position.is_short and crossover(vi_plus, vi_minus)) or \
#                (abs(fisher[-1]) > 2 and fisher[-1] * fisher[-2] < 0):
#                 
#                 self.position.close()
#                 print(f"🌙💡 VORTEX REVERSAL! Closing position @ {self.data.Close[-1]} 🛑")
# 
# bt = Backtest(data, VortexFisher, cash