...
#     def init(self):
#         # 📈 INDICATORS
#         self.middle_band = self.I(talib.SMA, self.data.Close, 20, name='MiddleBB')
#         self.std_dev = self.I(talib.STDDEV, self.data.Close, 20, name='StdDev')
...
#         self.volume_ma = self.I(talib.SMA, self.data.Volume, 20, name='VolumeMA')
...
#                       self.data.Volume[-1] < self.volume_ma[-1]):
#                     self.position.close()
#                     print(f"🌑 SL SHORT @ {self.data.Close[-1]:.2f} | Band Re-Entry 🛑")
# 
