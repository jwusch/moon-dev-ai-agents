...
#                     self.position.close()
#                     print(f"🌙✨ Short stopped out at {self.data.Close[-1]}! Moon defense active!")
#                 elif current_atr >= 2*self.entry_atr:
#                     self.position.close()
#                     print(f"🚀🌕 ATR doubled! Short profit taken at {self.data.Close[-1]}!")
#         
#         # Moon Dev entry signal generator 🌌
#         else:
#             if len(self.data.Close) < 50: return
#         
#             liq_ok = self.liquidations[-1] > self.liquidation_thresh
#             fund_ok = abs(self.funding[-1]) > self.funding_extreme
#             bbw_ok = self.bbw[-1] == min(self.bbw[-self.bbw_lookback:])
#             
#             if liq_ok and fund_ok and bbw_ok:
#                 if self.funding[-1] > self.funding_extreme and self.data.Close[-1] < self.lower_band[-1]:
#                     self._enter_short()
#                 elif self.funding[-1] < -