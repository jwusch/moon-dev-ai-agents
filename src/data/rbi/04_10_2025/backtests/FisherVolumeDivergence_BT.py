...
class FisherVolumeDivergence(Strategy):
    ...
    def init(self):
        ...
        # Volume-Weighted MACD (using Close*Volume as input)
        ...
        # Dynamic Support/Resistance (20-period Donchian)
        self.dynamic_resistance = self.I(talib.MAX, self.data.High, 20, name='RESISTANCE')
        self.dynamic_support = self.I(talib.MIN, self.data.Low, 20, name='SUPPORT')
...
#             print(f"🌙 Moon Dev Warning: Skipping cosmic dead zone at {current_time}")
    ...
    def check_long_conditions(self):
        ...
        if resistance_break:
            self.last_breakout = len(self.data)
            
        return (abs(self.last_fisher_cross - self.last_divergence) <= 3 and
                abs(self.last_fisher_cross - self.last_breakout) <= 3 and
                resistance_break)

    def enter_long(self):
        # Cosmic Position Sizing 🌠
        stop_price = self.dynamic