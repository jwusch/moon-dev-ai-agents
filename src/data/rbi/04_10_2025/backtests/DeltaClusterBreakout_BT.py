...
#         if not self.position and crossover(self.data.Close, self.swing_high):
...
#         
#         # Short Entry Nebula 🌌
#         elif not self.position and crossunder(self.data.Close, self.swing_low):
#             if self.cmo[-1] < -50 and self.cmo[-2] >= -50:
#                 risk_amount = self.risk_pct * self.equity
#                 stop_price = price + min_stop
...
#             if trade.is_long:
#                 new_sl = max(trade.sl or 0, self.s1[-1])
#                 if new_sl > trade.sl:
#                     trade.sl = new_sl
#                     print(f"🌙✨ Long stop lifted to {new_sl:.2f} | Riding Moon Waves!")
#             else:
#                 new_sl = min(trade.sl or float