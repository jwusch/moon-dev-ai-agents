...
#             print(f"🌙✨ Closing LONG at {self.data.Close[-1]} (Trailing Stop Hit!)")
#             self.position.close()
#             
#         if self.position.is_short and self.data.High[-1] >= current_upper:
#             print(f"🌙✨ Closing SHORT at {self.data.Close[-1]} (Trailing Stop Hit!)")
#             self.position.close()
#         
...
#         
#         # Long Entry
#         if current_cmo > current_sma and prev_cmo <= prev_sma and volume_condition:
...
#             print(f"🚀🌙 BULLISH BREAKOUT! Buying {size} @ {self.data.Close[-1]}")
#             self.buy(size=size)
#             
#         # Short Entry
#         elif current_cmo < current_sma and prev_cmo >= prev_sma and volume_condition:
#             risk = current_upper - self.data.Close[-1]
#             if risk <= 0:
#                 print("🌙⚠️ Invalid short risk - skipping trade")
#                 return
#             size = int(round((self.equity * self.risk_percent) / risk))
#             print(f"🌙🚀 BEARISH BREAKDOWN! Shorting {size} @ {self.data.Close[-1]}")
#             self.sell(size=size)
# 
