...
# Clean column names and drop unnamed columns with Moon Dev precision ✨
data.columns = data.columns.str.strip().str.lower()
data = data.drop(columns=[col for col in data.columns if 'unnamed' in col.lower()])

# Proper column mapping for celestial alignment 🌌
data.rename(columns={
    'open': 'Open',
    'high': 'High',
    'low': 'Low',
...
#     risk_pct = 0.01  # 1% risk per trade (Moon Dev Standard Risk Protocol) 🌙
    ...
    def init(self):
        # Moon Dev Indicators Setup with Celestial Precision 🌙✨
        ...
        # ATR for Moon-Sized Stops (Celestial Protection) 🌙🛡️
        self.atr = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, 14, name='ATR')
        
        # Volume SMA for Fuel Check (Rocket Fuel Verification) 🚀⛽
...
#             print("🌙⚡ Moon Dev Circuit Breaker: 3 Consecutive Losses! Trading Halted.")
        ...
        if not self.position:
            # Short Entry Constellation (Bearish Celestial Alignment) 🌙💫
            if self.data.High[-1] > self.upper_band[-1]:
                if (self.data.Volume[-1] < self.data.Volume[-2] and 
                    ...
                    if (self.data.High[-1] > self.data.High[-2] and 
                        ...
                        # Moon Position Sizing (Celestial Mathematics) 🌙📐
