"""
Moon Dev's LiquidationSurge Strategy
...
#                 if self.data.Close[-2] < swing_h[-2] and self.data.Close[-1] > swing_h[-1]:
#                     sl = self.data.Low[-1]
...
#                 if self.data.Close[-2] > swing_l[-2] and self.data.Close[-1] < swing_l[-1]:
#                     sl = self.data.High[-1]
#                     size = moon_size(price, sl)
#                     if size > 0:
