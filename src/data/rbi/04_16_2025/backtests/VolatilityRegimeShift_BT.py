"""
Moon Dev's VolatilityRegimeShift Strategy
...
#             current_bw = bw[-1]
#             current_sma = bw_sma[-1]
#             
#             # Take profit on bandwidth expansion
#             if (current_bw > current_sma) and (bw[-2] < bw_sma[-2]):
#                 self.position.close()
#                 print(f"🌙💫 COSMIC PROFIT TAKEN! Closed at {self.data.Close[-1]}")
# 
#             # Opposing signal exit
#             if is_long and short_trigger:
#                 self.position.close()
#                 print(f"🌙⚡ BEARISH ENERGY DETECTED! Bailing long at {self.data.Close[-1]}")
#             elif not is_long and long_trigger:
#                 self.position.close()
#                 print(f"🌙⚡ BULLISH VIBES D