...
class VolSpikeReversal(Strategy):
    ...
    def init(self):
        ...
        std_dev = self.I(talib.STDDEV, returns, self.volatility_window, name='STDDEV')
        volume_avg = self.I(talib.SMA, self.data.Volume, self.volatility_window, name='VOL_AVG')
        ...
        self.vol_weighted_vol_avg = self.I(talib.SMA, self.vol_weighted_vol, self.volatility_window, name='VOL_W_VOL_AVG')
...
#                 print(f"🌪️ MOON DEV VOL EXIT | Bar {current_bar} | Price: {self.data.Close[-1]:.2f}")
            ...
            sma_descending = self.sma20[-1] < self.sma20[-2]
...
#                     print(f"🚀 MOON DEV SHORT ENTRY | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss:.2f}")
