"""
Moon Dev's VortexChaikinCluster Strategy
...
# Here's the debugged version of your VortexChaikinCluster strategy with proper position sizing and technical fixes while preserving the original strategy logic:
# 
# ```python
# import pandas as pd
...
#                         print(f"🌙 MOON ROCKET LAUNCH! 🚀 LONG {size} units @ {current_close:.2f}"
#                               f" | SL: {stop_price:.2f} | CMF: {self.cmf[-1]:.2f}")
#             
#             # === SHORT ENTRY === 🌪️
#             vi_bearish = (self.vi_plus[-1] < self.vi_minus[-1] and 
#                         self.vi_plus[-2] >= self.vi_min