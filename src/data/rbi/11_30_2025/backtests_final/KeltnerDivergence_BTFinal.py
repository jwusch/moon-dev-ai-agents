"""
Moon Dev's KeltnerDivergence Strategy
...
# 
#     def init(self):
#         # Moon Debug 🌙: Strip whitespace from column names.
#         self.data.columns = self.data.columns.str.strip().str.lower()
#         
#         # Moon Debug 🌙: Drop unnamed columns if present.
#         self.data = self.data.drop(columns=[col for col in self.data.columns if 'unnamed' in col.lower()])
# 
#         # Moon Debug 🌙: Preparing indicators
#         close = self.data['close']
#         high = self.data['high']
#         low = self.data['low']
# 
#         # Moon Debug 🌙: Initialize indicators
...
#     def next(self):
#         if self.position:
#             # Moon Debug 🌙: Exiting logic
#             if self.data.close[-1] < self.ema20[-1]:
#                 self.position.close()
#                 print("🛑 Moon Exit: Price closed below KC basis! 📉")
# 
#         else:
#             # Moon Debug 🌙: Entry logic evaluation
#             close = self.data.close[-1]
#             kc_upper = self.kc_upper[-1]
# 
#             # Moon Debug 🌙: Check conditions
...
# 
#             # Moon Debug 🌙: Bullish divergence logic
...
#                 
#                 # Moon Debug 🌙: Calculating stop loss
#                 stop_loss = min(self.ema20[-1], entry_price - self.Atr_Multiplier * atr20_val)
#                 risk_amount = self.equity * self.Risk_Percent
# 
#                 # Moon Debug 🌙: Calculating position size
#                 position_size = risk_amount / abs(entry_price - stop_loss)
#                 position_size = max(1, int(round(position_size)))  # Ensure at least 1 unit
#                 self.buy(size=position_size)
# 
#                 print("✨🌙 Entered long position! All conditions satisfied! 🚀✨")
# 
# # Moon Debug 🌙: Load data
