...
class VolumeSurgeBreakout(Strategy):
    ...
    def init(self):
        ...
        self.vol_ma = self.I(talib.SMA, self.data.Volume, timeperiod=self.volume_window)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (price > self.resistance[-1] and
                ...
                if size > 0:  # Ensure positive position size
                    sl = price - atr
                    tp = price + 1.5*atr
                    self.buy(size=size, sl=sl, tp=tp, tag='LONG')
            ...
            # Short entry nebula 🌑📉    
                ...
                if size > 0:  # Ensure positive position size
                    sl = price + atr
                    tp = price - 1.5*atr
                    self.sell(size=size, sl=sl, tp=tp, tag='SHORT')
