...
class LiquidationReversion(Strategy):
    ...
    def init(self):
        ...
        self.volume_ma = self.I(talib.SMA, self.data.Volume, timeperiod=self.volume_ma_period, name='VOLUME MA')
    ...
    def next(self):
        ...
        if self.data.High[-1] == self.swing_high[-1]:
            ...
            print(f" NEW SWING HIGH DETECTED AT {self.data.index[-1]} (Price: {self.data.High[-1]})")
        if self.data.Low[-1] == self.swing_low[-1]:
            self.last_swing_low_idx = current_idx
            print(f" NEW SWING LOW DETECTED AT {self.data.index[-1]} (Price: {self.data.Low[-1]})")
