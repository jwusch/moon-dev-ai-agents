"""
Moon Dev's VolatilitySqueezeReversal Strategy
...
# from talib import BBANDS, MAX, MIN
# import pandas as pd
# import numpy as np
# 
...
#         # Bollinger Bands
#         self.upper_band = self.I(lambda close: BBANDS(close, self.bb_period, self.bb_dev, self.bb_dev)[0], self.data.Close)
#         self.middle_band = self.I(lambda close: BBANDS(close, self.bb_period, self.bb_dev, self.bb_dev)[1], self.data.Close)
#         self.lower_band = self.I(lambda close: BBANDS(close, self.bb_period, self.bb_dev, self.bb_dev)[2], self.data.Close)
#         
#         # Bollinger Band Width
#         self.bb_width = self.I(lambda u, l, m: (u - l)/m, self.upper_band, self.lower_band, self.middle_band)
#         
#         # Volatility Contraction
#         self.bb_width_min = self.I(MAX, self.bb_width, self.contraction_period)
#         self.bb_width_max = self.I(MIN, self.bb_width, self.contraction_period)
#         
#         # Funding Rate Extremes
#         self.funding_max = self.I(MAX, self.data.Funding_Rate, self.funding_lookback)
#         self.funding_min = self.I(MIN, self.data.Funding_Rate, self.funding_lookback)
#         
#     def next(self):
...
#                 print(f"🌙✨ Closing LONG at {price:.2f} (Middle Band)")
#                 trade.close()
#             elif trade.is_short and price >= middle:
#                 print(f"🌙✨ Closing SHORT at {price:.2f} (Middle Band