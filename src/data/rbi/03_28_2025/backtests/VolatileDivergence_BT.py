"""
Moon Dev's VolatileDivergence Strategy
...
#         self.low_min = self.I(talib.MIN, self.data.Low, self.rsi_period)
#         self.atr5 = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, self.atr_short)
#         self.atr20 = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, self.atr_long)
#         self.sma10 = self.I(talib.SMA, self.data.Close, self.sma_exit)
#         
...
#                 print(f"🚀🌕 LONG ENTRY! Size: {position_size} @ {current_close:.2f} | SL: {sl_price:.2f}")
#         
#         # Moon Dev Exit Logic 📉
#         if self.position and current_close