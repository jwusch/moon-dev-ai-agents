...
class BandwidthDivergence(Strategy):
    ...
    def init(self):
        ...
        def bb_upper(close):
            u, _, _ = talib.BBANDS(close, 20, 2, 2)
            return u
        def bb_middle(close):
            _, m, _ = talib.BBANDS(close, 20, 2, 2)
            return m
        def bb_lower(close):
            _, _, l = talib.BBANDS(close, 20, 2, 2)
            return l
        ...
        self.chaikin = self.I(talib.ADOSC, self.data.High, self.data.Low, 
                             self.data.Close, self.data.Volume, 3, 10, name='CHAIKIN')

        # Bandwidth calculations 📈
        self.bandwidth = self.I(lambda: (self.bb_upper - self.bb_lower)/self.bb_middle, 
                              name='BANDWIDTH')
        self.bw_min = self.I(talib.MIN, self.bandwidth, self.bw_lookback, name='BW_MIN')
        self.bw_max = self.I(talib.MAX, self.bandwidth, self.bw_lookback, name='BW_MAX')

        # Volume and swing calculations 🌊
        self.vol_ma = self.I(talib.SMA, self.data.Volume, self.volume_ma_period, name='VOL_MA')
        self.price_low = self.I(talib.MIN, self.data.Low, self.swing_period, name='SWING_LOW')
        self.ck_low = self.I(talib.MIN, self.chaikin, self.swing_period, name='CK_LOW')
