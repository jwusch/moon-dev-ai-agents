...
#                     print(f"🌙✨🚀 MOON DEV LONG! Size: {size} | Entry: {price:.2f} | SL: {sl:.2f}")
#             
#             # Short Entry 🌑    
#             elif (bb_cond and cmf < -0.2 and 
#                   self.data.Low[-1] < self.prior_low[-1]):
#                 sl = price + 1.5*atr
#                 risk_amount = self.equity * self.risk_pct
#                 size = int(round(risk_amount/(sl - price))) if sl > price else 0
#                 if size: