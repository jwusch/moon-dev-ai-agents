...
#         width = (self.kc_upper - self.kc_lower)/self.kc_mid
#         self.width_20 = self.I(ta.percentile, width, length=self.percentile_lookback, percentile=20, name='WIDTH_20')
#         self.width_80 = self.I(ta.percentile, width, length=self.percentile_lookback, percentile=80, name='WIDTH_80')
# 
...
#             vwma_cross = crossover(self.data.Close, self.vwma)
...
#                         print(f"🌙🚀 MOON ENTRY! Size: {position_size} | Risk: {self.risk_pct*100}% | SL: {stop_price:.2f}")
# 
# # Run backtest with Moon Dev parameters 🌙
# bt = Backtest(data, VolumetricSqueeze, cash=1_000_000, commission=.002)
# stats = bt.run()
# print(stats