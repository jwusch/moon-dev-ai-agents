...
class VolatilityBreakout(Strategy):
    ...
    def next(self):
        ...
        if self.prev_ci is not None and self.prev_vwma is not None:
            ...
            if (self.prev_ci >= 61.8 and ci < 61.8) and (self.data.Close[-2] > self.prev_vwma):
...
#                         print(f"🌙✨🚀 MOON DEV SHORT ENTRY | Size: {size} | Entry: {self.data.Open[-1]:.2f} | SL: {sl:.2f} | TP: {self.data.Open[-1] - 2*risk_per_share:.2f}")