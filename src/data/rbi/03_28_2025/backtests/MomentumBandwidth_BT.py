"""
Moon Dev's MomentumBandwidth Strategy
...
#             if (prev_cmo < self.cmo_threshold_long and current_cmo >= self.cmo_threshold_long and
...
#             elif (prev_cmo > self.cmo_threshold_short and current_cmo <= self.cmo_threshold_short and
#                   self.bandwidth[-1] <= self.bandwidth_low[-1]):
#                 
#                 atr_value = self.atr[-1]
#                 stop_loss = current_close + (self.atr_multiplier * atr_value)
...
#                              tag="🌙 SHORT: CMO Breakdown + Vol Squ