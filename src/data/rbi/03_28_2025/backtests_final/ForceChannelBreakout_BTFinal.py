"""
Moon Dev's ForceChannelBreakout Strategy
...
#             crossover(self.fi, 0),  # Proper crossover check
...
#                         tp=tp_level,
#                         tag="MoonRocket")
#                 print(f"🚀 LIFTOFF! Long {position_size} units @ {entry_price:.2f} | SL: {final_stop:.2f} | TP: {tp_level:.2f}")
# 
#         # 🌒 Exit Black Hole 🌒
#         elif self.position and crossunder(self.fi, 0):  # Proper crossunder check
#             self.position.close()
#             print(f"🌑 GRAVITY WELL! Exiting @ {price:.2f}")
# 
# # 🌟 Launch Backtest 🌟
# bt = Backtest(data, ForceChannelBreakout, cash=1_000_000, commission=.002, trade_on_close=True)
# stats = bt.run()
# 
