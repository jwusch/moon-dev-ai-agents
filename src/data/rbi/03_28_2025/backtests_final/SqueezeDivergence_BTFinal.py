...
class SqueezeDivergence(Strategy):
    ...
    def init(self):
        ...
        self.bb_squeeze_threshold = self.I(lambda x: x.rolling(35000).quantile(0.05), 
                                   self.bb_width, 
                                   name='BB_Squeeze_Threshold')
        
        # Calculate Swing Highs for price and volume
        self.price_swing_high = self.I(talib.MAX, 
                                     self.data.High, 
                                     timeperiod=20, 
                                     name='Price_Swing_High')
        self.volume_swing_high = self.I(talib.MAX, 
                                      self.data.Volume, 
                                      timeperiod=20, 
                                      name='Volume_Swing_High')
        ...
        self.bb_width_ma50 = self.I(talib.SMA, 
                                   self.bb_width, 
                                   timeperiod=50, 
                                   name='BB_Width_MA50')
...
#                 print(f"🌙✨ Moon Dev Exit Signal! Volatility expanded at {self.data.Close[-1]:.2f}")
        ...
        if squeeze_condition:
            ...
            # Check for bearish divergence (price higher high but volume