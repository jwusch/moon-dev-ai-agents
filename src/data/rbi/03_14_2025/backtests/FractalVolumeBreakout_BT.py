"""
Moon Dev's FractalVolumeBreakout Strategy
...
#             print(f"🌙✨ New RESISTANCE formed at {self.last_resistance:0.2f}")
# 
#         current_min = self.min_low[-1]
#         fractal_low = self.data.Low[-3]
#         if fractal_low == current_min:
#             self.last_support = fractal_low
#             print(f"🌙✨ New SUPPORT formed at {self.last_support:0.2f}")
# 
...
#                     print(f"🚀🌕 BULLISH BREAKOUT! Resistance: {self.last_resistance:0.2f}, Volume: {self.data.Volume[-1]:0.2f} (VMA: {self.vma[-1]:0.2f})")
#                     self.enter_long()
# 
#             # Short entry: Crash support with meteor volume ☄️
#             elif self.last_support and self.data.Low[-1] < self.last_support:
#                 if self.data.Volume[-1] > self.volume_multiplier * self.vma[-1]:
#                     print(f"🚀🌑 BEARISH BREAKOUT! Support: {self.last_support:0.2f}, Volume: {self.data.Volume[-1]:0.2f} (VMA: {self.vma[-1]:0.2f})")
#                     self.enter_short()
# 
...
#             print(f"🌕💫 LONG POSITION: {position_size} units at {entry_price:0.2f} (SL: {sl_price:0.2f})")
# 
#     def enter