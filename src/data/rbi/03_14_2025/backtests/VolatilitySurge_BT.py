...
#                 print(f"🚀 MOON DEV LONG SIGNAL @ {current_close:0.2f}")
#                 print(f"✨ Size: {position_size} | SL: {stop_loss:0.2f} | TP: {current_close + (self.take_profit_multiplier * atr_value):0.2f}")
...
#                 
#                 # Moon Dev Entry Signal 🌙
#                 print(f"🌑 MOON DEV SHORT SIGNAL @ {current_close:0.2f}")
#                 print(f"✨ Size: {position_size} | SL: {stop_loss:0.2f} | TP: {current_close - (self.take_profit_multiplier * atr_value):0.2f}")
...
#         elif self.position.is_long and self.data.Close[-1] > self.position.entry_price:
#             new_sl = self.data.Close[-1] - (self.atr_multiplier * atr_value)
#             self.position.sl = max(new_sl,