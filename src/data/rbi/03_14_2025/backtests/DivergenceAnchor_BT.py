"""
Moon Dev's DivergenceAnchor Strategy
...
#         if or der.status in [or der.Completed]:
#             if or der.isbuy():
#                 self.log(f'🚀 BUY EXECUTED - Size: {or der.size} @ {or der.executed.price:0.2f}')
#             elif or der.issell():
#                 self.log(f'🌑 SELL EXECUTED - Size: {or der.size} @ {or der.executed.price:0.2f}')
# 
#     def next(self):
...
#                     self.log(f'🔻 BEARISH DIVERGENCE DETECTED! Price ↗️ {current_high:0.2f} vs {last_high:0.2f}, MACD ↘️ {self.macd_line[0]:0.2f} vs {last_macd:0.2f}')
#                     
...
#             self.log(f'✨ PRICE BOUNCE CONFIRMED @ {support_level:0.2f}')
...
#         
#         if risk_per_unit <= 0:
#             self.log('⚠️ INVALID RISK CALCULATION - Trade skipped')
#             return
#             
