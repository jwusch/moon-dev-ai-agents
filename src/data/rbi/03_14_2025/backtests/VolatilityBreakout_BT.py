...
#             print(f"🌕 MOON DEV UPDATE 🌙 | Bar: {len(self.data)} | Price: {self.data.Close[-1]:0.2f} | Equity: {self.equity:,0.2f}")
#         
...
#                 print(f"\n🚀 MOONSHOT ALERT 🌕 | ENTRY: {entry_price:0.2f} | SIZE: {position_size} | RISK: {atr_value:0.2f} | TP: {take_profit:0.2f} | SL: {stop_loss:0.2f}")
#         else:
#             # Exit Conditions 🌑
#             if self.data.Close[-1] < self.bb_middle[-1]:
#                 self.position.close()
#                 print(f"\n🌑 LUNAR ECLIPSE EXIT 🌘 | CLOSE: {self.data.Close[-1]:0.2f} | REASON: Middle Band Break")
# 
# # Launch Backtest 🌕✨
