...
class BandDivergence(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(lambda c: talib.SMA(c, 20) + 2*talib.STDDEV(c, 20), self.data.Close)
        self.bb_lower = self.I(lambda c: talib.SMA(c, 20) - 2*talib.STDDEV(c, 20), self.data.Close)
