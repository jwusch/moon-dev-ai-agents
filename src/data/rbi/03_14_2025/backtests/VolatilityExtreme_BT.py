"""
Moon Dev's VolatilityExtreme Strategy
...
#                         print(f"🌙🚀 MOON DEV LONG: {current_close:0.2f} < 3σ {lower_3std:0.2f} | SL: {stop_loss:0.2f} | Size: {position_size}")
# 
...
#                         self.entry_bar = len(self.data)-1
#                         print(f"🌙📉 MOON DEV SHORT: {current_close:0.2f} > 2σ {upper_2std:0.2f} | SL: {stop_loss:0.2f} | Size: {position_size}")
#         else:
...
#                     self.position.close()
#                     print(f"🌙✨ MOON DEV LONG EXIT: {current_close:0.2f} >= 1σ {exit_level:0.2f}")
#             elif self.position.is_short:
#                 exit_level = current_sma + current_std
#                 if current_close <= exit_level:
#                     self.position.close()
#                     print(f"🌙✨ MOON DEV SHORT EXIT: {current_close:0.2f} <= 1σ {exit_level:0.2f}")
# 
# # 🌙 DATA PREPARATION
