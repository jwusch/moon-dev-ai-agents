...
#                 self.trade_count += 1
#                 print(f"\n🚀 MOON SHOT! Long {position_size} @ {current_close:0.2f}")
#                 print(f"   🌗 RSI Divergence: {prev_rsi_high:0.2f} -> {current_rsi:0.2f}")
#                 print(f"   📉 Volume Below SMA: {volume_sma:0.2f} vs {current_volume:0.2f}")
# 
#                 # 🎯 Exit Targets
...
#                 print(f"   🔐 Stop Loss: {stop_price:0.2f} | 🎯 Take Profit: {profit_price:0.2f}")
# 
#         # 🌊 Exit Condition - RSI Convergence
#         if self.position and current_rsi > self.exit_rsi_level:
#             self.position.close()
#             print(f"\n🌑 RSI CONVERGENCE! Closing position @ {current_close:0.2f}")
