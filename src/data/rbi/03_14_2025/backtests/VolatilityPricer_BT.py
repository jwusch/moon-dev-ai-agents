"""
Moon Dev's VolatilityPricer Strategy
...
#                 print(f"🌧️🚨 MOON DEV STOP LOSS! Closing {self.position.size} units @ {current_close:0.2f}")
#                 self.position.close()
#             elif current_close >= self.take_profit:
#                 print(f"🎯🚀 MOON DEV TAKE PROFIT! Closing {self.position.size} units @ {current_close:0.2f}")
#                 self.position.close()
#             elif current_close < self.bb_lower[-1]:
#                 print(f"🔻🌙 MOON DEV EXIT: Price breached Lower BB @ {current_close:0.2f}")
#                 self.position.close()
#             elif self.rsi[-1] < 30 and self.rsi[-2] >= 30:
#                 print(f"📉🌑 MOON DEV EXIT: RSI divergence @ {self.rsi[-1]:0.2f}")
#                 self.position.close()
# 
...
#                 # 🎲 MERTON'S PROBABILITY CALCULATIONS
#                 daily_vol = (self.hv[-1]/