"""
Moon Dev's DivergentCrossover Strategy
...
#                         print(f"🌙 BEARISH DIVERGENCE! Price ↗ {self.swing_high[-1]:0.2f} vs {prev['price']:0.2f}, RSI ↘ {self.rsi[-1]:0.2f} vs {prev['rsi']:0.2f}")
#             
...
#                         print(f"🌙 BULLISH DIVERGENCE! Price ↘ {self.swing_low[-1]:0.2f} vs {prev['price']:0.2f}, RSI ↗ {self.rsi[-1]:0.2f} vs {prev['rsi']:0.2f}")
# 
...
#                     print(f"🚀🌕 BULLISH ENTRY | Size: {size} | SL: {sl:0.2f} | RSI: {self.rsi[-1]:0.2f} CCI: {self.cci[-1]:0.2f}")
#             
#             # Bearish entry logic
#             elif bearish_divergence and macd_death and self.hist[-