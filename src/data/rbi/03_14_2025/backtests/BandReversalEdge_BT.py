"""
Moon Dev's BandReversalEdge Strategy
...
#                     print(f"🌙 Moon Dev Entry Signal 🚀 | Price: {price:0.2f} | Size: {position_size} | SL: {self.stop_loss_price:0.2f} | Equity: {equity:0.2f}")
#         else:
#             # Exit conditions
#             exit_price = None
...
#                 self.position.close()
#                 print(f"🎯 Moon Dev Profit Target Hit! | Close: {self.data.Close[-1]:0.2f}")
#                 return
#                 
#             # Stop loss check
#             if self.data.Low[-1] <= self.stop_loss_price:
#                 self.position.close()
#                 print(f"🛑 Moon Dev Stop Loss Triggered! | Close: {self.data.Close[-1]:0.2f}")
#                 return
#                 
#             # Time-based exit
#             if current_bar - self.entry_bar >= self.exit_bars:
#                 self.position.close()
#                 print(f"⌛ Moon Dev Time Exit | Bars Held: {current_bar - self.entry_bar} | Close: {self.data.Close[-