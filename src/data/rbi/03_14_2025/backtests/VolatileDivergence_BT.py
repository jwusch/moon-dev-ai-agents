...
#                     if crossover(self.data.Close, self.swing_high):
#                         risk = self.data.Close[-1] - self.swing_lows[-1]
#                         size = int(round((self.equity * self.risk_per_trade) / risk))
#                         self.buy(size=size, 
#                                 sl=self.swing_lows[-1] * 0.999,
#                                 tp=self.data.Close[-1] + risk)
#                         print(f"🚀 BULLISH ENTRY | Size: {size} | R:R 1:1")
#                         
#                 elif bear_div and self.data.Close[-1]