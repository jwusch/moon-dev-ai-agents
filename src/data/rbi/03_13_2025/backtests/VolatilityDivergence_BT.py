"""
Moon Dev's VolatilityDivergence Strategy
...
#         self.macd, self.macd_signal, self.macd_hist = self.I(talib.MACD, self.data.Close, 12, 26, 9)
...
#         self.macd_swing_high = self.I(talib.MAX, self.macd, self.swing_period)
#         self.macd_swing_low = self.I(talib.MIN, self.macd, self.swing_period)
#         self.rsi_swing_high = self.I(talib.MAX, self.rsi, self.swing_period)
#         self.rsi_swing_low = self.I(talib.MIN, self.rsi, self.swing_period)
# 
#         # Track swing points
#         self.price_lows = []
#         self.macd_lows = []
...
#         if current_rsi == self.rsi_swing_high[-1]:
#             self.rsi_highs.append((len(self.data)-1, current_rsi))
