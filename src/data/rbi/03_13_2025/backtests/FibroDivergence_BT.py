"""
Moon Dev's FibroDivergence Strategy
...
#         self.price_swing_high = self.I(talib.MAX, self.data.High, self.divergence_window)
#         self.price_swing_low = self.I(talib.MIN, self.data.Low, self.divergence_window)
#         self.rsi_swing_high = self.I(talib.MAX, self.rsi, self.divergence_window)
#         self.rsi_swing_low = self.I(talib.MIN, self.rsi, self.divergence_window)
# 
#     def next(self):
#         if self.position:
...
#                 if position_size > 0:
#                     print(f"🌙✨ BULLISH CONFIRMATION! Buying {position_size} units at {close:.2f}")
#                     self.buy(size=position_size, sl=sl, tp=close + at