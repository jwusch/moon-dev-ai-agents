...
class VolatilityPulse(Strategy):
    ...
    def init(self):
        ...
        # Core indicators
        self.atr = self.I(talib.ATR, h, l, c, 14, name='ATR_14')
        self.atr_avg = self.I(talib.SMA, self.atr, 14, name='ATR_Avg')
        self.ema50 = self.I(talib.EMA, c, 50, name='EMA_50')
        self.atr_20_avg = self.I(talib.SMA, self.atr, 20, name='ATR_20_Avg')
...
#             print(f"🌫️ Moon Dev: Low volatility filter active (ATR {current_atr:.2f} < 20MA)")
        ...
        if not self.position:
            ...
            # Short conditions
                ...
                bear_signal = self.engulf[-1] == -100 or self.shooting[-1] == 100
                if bear_signal:
                    self.enter_short()
        
        # Exit logic
        if self.position.is_long:
            self.manage_long_exits()
