"""
Moon Dev's RetracementContra Strategy
...
#             self.position.close()
#             print(f"🚨 MOON DEV ALERT: 20% Max Drawdown Triggered! Equity: {self.equity:,.2f}")
#             return
# 
#         if self.position:
...
#                 self.position.close()
#                 print(f"🌙✨ Exit Signal: Price broke ATR bands! Closing at {self.data.Close[-1]:.2f}")
#         else:
#             # 🎯 ENTRY CONDITIONS
...
#             atr_contract = current_atr < np.nanmean(self.atr[-self.atr_contract_lookback:])
#             
#             # 💡 ENTRY LOGIC
...
#                     print(f"🌙🚀 ENTRY: {position_size} units at {self.data.Close[-1]:.