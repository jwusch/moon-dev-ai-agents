...
#         print(f"🌙 Current Close: {current_close:0.2f} | ATR: {current_atr:0.2f} | ATR Ratio: {atr_ratio:0.2f}")
#         
#         if not self.position:
#             # Long entry conditions
#             ema_cross_above = (self.ema[-2] <= self.sma[-2]) and (self.ema[-1] > self.sma[-1])
#             macd_delta_below = self.macd_delta[-1] < self.macd_delta_ma[-1]
#             atr_ratio_rising = self.atr_ratio[-1] > self.atr_ratio[-2]
#             
#             # Short entry conditions
#             ema_cross_below = (self.ema[-2] >= self.sma[-2]) and (self.ema[-1] < self.sma[-1])
...
#                 position_size = int(round(risk_amount / (current