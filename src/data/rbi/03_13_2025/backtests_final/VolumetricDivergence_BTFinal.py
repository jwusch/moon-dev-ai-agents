...
class VolumetricDivergenceStrategy(Strategy):
    ...
    def init(self):
        ...
        self.vo_signal = self.I(lambda: talib.SMA(self.volume_osc, 9), name='VO Signal')
        
        # RSI and MACD
        self.rsi = self.I(lambda: talib.RSI(close, 14), name='RSI')
...
#         print(f"\n🌙 Moon Dev Status Bar [{self.data.index[-1]}]")
        ...
        bull_div = (
            ...
            (self.volume_osc[-2] < self.vo_signal[-2] and self.volume_osc[-1] > self.vo_signal[-1]) and
            (self.rsi[-1] > 30) and
            (self.macd_hist[-1] > 0)
        )
