"""
Moon Dev's InverseMomentumPut Strategy
...
# I've analyzed the code and found a few technical issues that need fixing. Here's the corrected version with Moon Dev themed debug prints and proper position sizing:
# 
# ```python
# # -*- coding: utf-8 -*-
# import pandas as pd
...
#             bearish_crossover = (self.sma3[-2] >= self.sma9[-2] and 
#                                 self.sma3[-1] < self.sma9[-1])
#             
...
#                 risk_per_share = abs(stop_loss_price - current_close)
#                 
#                 if risk_per_share <= 0:
...
#                     print(f"🌙✨ SHORT ENTERED! Size: {position_size} @ {current_close:0.2f}")
#                     print(f"   🛑 Stop Loss: {self.stop_loss_level:0.2f} | 📉 Risk: {self.risk_per_trade*100}%")
#         
#         # Exit conditions
#         else:
#             # Check for bullish crossover
#             bullish_crossover = (self.sma3[-2] <= self.sma9[-2] and 
#                                 self.sma3[-1] > self.sma9[-1])
#             
#             # Check RSI undersold
...
#                 self.position.close()
#                 print(f"🚀 EXIT TRIGGERED! Closing @ {current_close:0.2f}")
