...
# Here's the fully debugged code with Moon Dev themed improvements while maintaining all original strategy logic:
# 
# ```python
...
#                     print(f"🚀🌙 MOON BLASTOFF! LONG {position_size} units @ {price:.2f} | SL: {stop_loss:.2f}")
#             
#             elif short_cond:
...
#                     print(f"🌧️🌙 SHORT SURGE! {position_size} units @ {price:.2f} | SL: {stop_loss:.2f}")
#         
#         # 🌙 Trailing Stop Logic
#         else:
#             if self.position.is_long:
#                 new_sl = max(self.position.sl, self.data.Low[-1] - (self.upper[-1] - self.lower[-1]))
#                 self.position.sl = new_sl
#             elif self.position.is_short:
#                 new_sl = min(self.position.sl, self.data.High[-1] + (self.upper[-1] - self.lower[-1]))
#                 self.position.sl = new_sl
# 
# # 🌙 Launch Backtest
