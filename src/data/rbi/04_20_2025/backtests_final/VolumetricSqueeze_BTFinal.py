...
class VolumetricSqueeze(Strategy):
    def init(self):
        ...
        self.bandwidth_min = self.I(lambda x: talib.MIN(x, timeperiod=20), self.bandwidth)
        
        # Volume Percentile
        self.volume_pct = self.I(lambda x: ta.percentile_rank(x, length=100), self.data.Volume)
