...
class VolatilitySqueezeDivergence(Strategy):
    ...
    def init(self):
        ...
        # ATR and Percentile
        atr = talib.ATR(self.data.High, self.data.Low, self.data.Close, self.atr_period)
        self.atr = self.I(lambda: atr, name='ATR')
        self.atr_pct = self.I(lambda: atr.rolling(self.lookback).rank(pct=True)*100, name='ATR_Pct')
        
        # Swing Highs/Lows
        self.price_high = self.I(talib.MAX, self.data.High, self.swing_window, name='Price_High')
        self.vwap_high = self.I(talib.MAX, self.vwap, self.swing_window, name='VWAP_High')
        self.price_low = self.I(talib.MIN, self.data.Low, self.swing_window, name='Price_Low')
        self.vwap_low = self.I(talib.MIN, self.vwap, self.swing_window, name='VWAP_Low')
        
        # Volume-Weighted RSI
        rsi = talib.RSI(close, 14)
        vw_rsi = rsi * self.data.Volume
        self.vw_rsi = self.I(lambda: talib.SMA(vw_rsi, 5), name='VW_RSI')
            ...
            if self.bb_width_pct[i] < 20:
                ...
                if self.atr_pct[i] > 80:
                    ...
                    if bull_div and price > vwap:
                        ...
                        size = int(round((self.equity*self.risk_pct)/(price-sl)))
                        if size > 0:
                            self.buy