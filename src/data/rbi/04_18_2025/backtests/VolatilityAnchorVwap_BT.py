"""
Moon Dev's VolatilityAnchorVwap Strategy
...
#             print(f"🌙 Processing bar {len(self.data)}/{len(self.data)}")
#         
...
#             print(f"🚀 Volume spike detected! {self.data.Volume[-1]:.2f} vs {self.vol_ma[-1]:.2f}")
...
#             vwap = (current_cum_typ - anchor['cum_typ'])/(current_cum_vol - anchor['cum_vol'])
#             
#             # Breakout entry
#             if (anchor['age'] <= 3 and 
#                 self.data.Close[-1] > vwap and 
#                 self.data.Close[-2] <= vwap and 
#                 self.data.Volume[-1] > 1.5*self.vol_ma