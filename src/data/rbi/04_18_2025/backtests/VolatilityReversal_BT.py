"""
Moon Dev's VolatilityReversal Strategy
...
#         sum_cl_vol = self.I(talib.SUM, close * volume, self.vw_period)
#         sum_vol = self.I(talib.SUM, volume, self.vw_period)
#         self.vwap = sum_cl_vol / sum_vol
#         
#         sq_diff = (close - self.vwap) ** 2 * volume
#         sum_sq_diff = self.I(talib.SUM, sq_diff, self.vw_period)
...
#         
#         # Fisher Transform
#         self.fisher = self.I(lambda h, l: pd_ta.fisher(high=h, low=l, length=9)[0], high, low)
#         
...
#                         print(f"🌙✨ LONG! Entry: {current_close:.2f} | SL: {sl:.2f} | TP: {tp:.2f} | Size: {size}")
#             
...
#                         print(f"🌙✨ SHORT! Entry: {current_close:.2f} | SL: {sl:.2f} | TP: {tp:.2f} | Size: {size}")
#         
#         # Time-based exit
#         else:
#             entry_bar = self.position.entry_bar
#             if len(self.data) - entry_bar >= self.time_exit_bars:
#                 self.position.close()
#                 print(f"🌙