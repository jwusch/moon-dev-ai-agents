"""
Moon Dev's DynamicVwapBreakout Strategy
...
#             atr_entry = self.atr[entry_bar]
#             
#             if trade.is_long:
#                 sl = trade.entry_price - atr_entry
#                 tp = trade.entry_price + 1.5*atr_entry
#                 
#                 if self.data.Low[-1] <= sl:
#                     trade.close()
#                     print(f"🌧️ SL HIT | Long closed @ {sl:.2f}")
#                 elif self.data.High[-1] >= tp:
#                     trade.close()
#                     print(f"🎯 TP HIT | Long closed @ {tp:.2f}")
#                 elif self.data.Close[-1] < self.upper_band[-1]:
#                     trade.close()
#                     print(f"📉 ENVELOPE EXIT | Long closed @ {self.data.Close[-1]:.2f}")
#                     
#             elif trade.is_short: