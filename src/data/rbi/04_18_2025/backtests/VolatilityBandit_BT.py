...
class VolatilityBandit(Strategy):
    ...
    def init(self):
        ...
        self.rsi14 = self.I(talib.RSI, self.data.Close, self.rsi_period)
        self.vol_sma5 = self.I(talib.SMA, self.data.Volume, self.vol_sma_period)
...
#                         print(f"\n🌑🌙 MOON DEV SHORT ENTRY! 💫 Size: {position_size} BTC")
        ...
        if self.position:
            ...
            if self.position.is_long:
                rsi_cross = crossunder(self.rsi14, 50)
                vol_decline = self.vol_sma5[-1] < self.vol_sma5[-2] if len(self.vol_sma5) > 1 else False
