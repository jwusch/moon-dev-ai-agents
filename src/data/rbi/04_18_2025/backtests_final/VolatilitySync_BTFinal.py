...
class VolatilitySync(Strategy):
    ...
    def next(self):
        ...
        if (not self.position and
            (self.efi[-2] < self.efi_std[-2] and self.efi[-1] > self.efi_std[-1]) and  # Bullish crossover
        ...
        # 🌑 SHORT ENTRY CONDITIONS
              ...
              (self.efi[-2] > -self.efi_std[-2] and self.efi[-1] < -self.efi_std[-1]) and  # Bearish crossunder
              self.bb_width[-1] < self.bb_width_avg[-1] and
              current_close < self.sma200[-1]):
