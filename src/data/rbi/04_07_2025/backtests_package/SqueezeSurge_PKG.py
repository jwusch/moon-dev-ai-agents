...
#     
#     def init(self):
#         # 🌙 CORE INDICATORS 🌙 (All using proper talib/pandas-ta)
...
#         self.bb_width = self.I(lambda x, y: x - y, self.bb_upper, self.bb_lower)  # Fixed width calculation
...
#         
#         # 🌙 SIGNAL CONDITIONS (No backtesting.lib used) 🌙
#         squeeze_condition = self.bb_width[-1] <= self.bb_squeeze[-1]
#         volume_surge = self.data.Volume[-1] > self.volume_sma50[-1]
#         
#         # 🚀 LONG SIGNAL 🌙
#         if not self.position.is_long and (ema_prev < ema_current) and squeeze_condition and volume_surge:
#             sl = price * 0.99
...
#         
#         # 🌙 SHORT SIGNAL 🌙
#         elif not self.position.is_short and (ema_prev > ema_current) and squeeze_condition and volume_surge:
#             sl = price * 1.01
#             risk_amount = self.equity * self.risk_pct
...
#         
#         # 🌙 EXIT CONDITIONS 🌙
#         if self.position.is_long and (ema_prev > ema_current):  # Fixed EMA crossover detection
#             self.position.close()
#             print(f"🌙💤 Closing