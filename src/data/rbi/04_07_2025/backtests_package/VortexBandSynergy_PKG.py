"""
Moon Dev's VortexBandSynergy Strategy
...
#                 if (self.vi_minus[-2] < self.vi_plus[-2] and self.vi_minus[-1] > self.vi_plus[-1]) or self.bb_width[-1] < self.bb_width_sma[-1]*0.8:
#                     self.position.close()
#                     print(f"🌑 Moon Dev Exit LONG: Signals reversed at {self.data.Close[-1]} ✨")
#             elif self.position.is_short:
#                 if (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) or self.bb_width[-1] < self.bb_width_sma[-1]*0.8:
#                     self.position.close()
...
#             
#             # Long Conditions ✅
#             long_trigger = ((self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) and
#                           (self.bb_width[-1] > self.