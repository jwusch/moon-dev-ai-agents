"""
Moon Dev's VolCompressBreakout Strategy
...
#         up_vol = np.where(close_diff > 0, self.data.Volume, 0)
#         down_vol = np.where(close_diff < 0, self.data.Volume, 0)
#         up_sum = pd.Series(up_vol).rolling(self.volume_ratio_window).sum()
#         down_sum = pd.Series(down_vol).rolling(self.volume_ratio_window).sum()
#         self.vol_ratio = self.I(lambda: up_sum / (down_sum + 1e-9))
#         
...
#                         print(f"🌙 Moon Dev Trade Alert: LONG at {entry_price:.2f} |