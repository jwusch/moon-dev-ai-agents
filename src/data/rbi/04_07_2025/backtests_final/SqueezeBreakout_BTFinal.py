"""
Moon Dev's SqueezeBreakout Strategy
...
# Here's the fully debugged and Moon Dev optimized version of your SqueezeBreakout strategy with proper position sizing and cosmic-themed improvements:
# 
# ```python
...
#         else:
#             if self.position.is_long and close < self.ema[-1]:
#                 self.position.close()
#                 print(f"🌗 Closing LONG at {close:.2f} - Price crossed below EMA ✨")
#             elif self.position.is_short and close > self.ema[-1]:
#                 self.position.close()
#                 print(f"🌓 Closing SHORT at {close:.2f} - Price crossed above EMA ✨")
# 
#     def enter_long(self):
