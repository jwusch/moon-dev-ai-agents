"""
Moon Dev's VortexVolumeDivergence Strategy
...
# Here's the fixed backtest code with proper position sizing and Moon Dev themed debug prints:
# 
# ```python
# import pandas as pd
# import talib
...
#         long_entry = (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) and (volume < volume_ema) and (adx_value < 20)
#         short_entry = (self.vi_minus[-2] < self.vi_plus[-2] and self.vi_minus[-1] > self.vi_plus[-1]) and (volume < volume_ema) and (adx_value < 20)
#         
#         long_exit = (self.vi_minus[-2] < self.vi_plus[-2] and self.vi_minus[-1] > self.vi_plus[-1]) or (adx_value > 25)
#         short_exit = (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) or (adx_value > 25)
#         
...
#                     if position_size > 0:
#                         print(f"🚀🌕 BLAST OFF LONG! | Size={position_size} | Entry={self.data.Close[-1]:.2f} | SL={stop_loss:.2f}")
