"""
Moon Dev's VortexChopFlow Strategy
...
#         
#         # 🚀 Long Entry Logic
#         if not self.position and crossover(self.vi_plus, self.vi_minus):
...
#                     if position_size > 0:
#                         print(f"🌙🚀 BULLISH CROSSOVER! Buying {position_size} units at {entry_price} ✨")
#                         self.buy(size=position_size, sl=stop_loss)
#         
#         # 🐻 Short Entry Logic
#         if not self.position and crossover(self.vi_minus, self.vi_plus):
#             if trend_condition and volume_condition:
#                 atr_value = self.atr[-1]
#                 entry_price = self.data.Close[-1]
#                 stop_loss = entry_price + 2 * atr_value
...
#                     position_size = int(round((self.equity * self.risk_per_trade) / risk_per_share))
#                     if position_size > 0:
#                         print(f"🌙📉 BEARISH CROSSOVER! Shorting {position_size} units at {entry_price} ✨")
...
#                 # Long exit triggers
#                 exit1 = crossover(self.vi_minus, self.vi_plus)
#                 exit2 = self.data.Close[-1] < self.swing_low[-1]
#                 if exit1 or exit2 or chop_exit_condition:
#                     print(f"🌙🛑 Closing LONG Position at {self.data.Close[-1]} (Choppiness: {self.choppiness[-1]:.1f})")
#                     self.position.close()
#             
#             elif self.position.is_short:
#                 # Short exit triggers
#                 exit1 = crossover(self.vi_plus, self.vi_minus)
#                 exit2 = self.data.Close[-1] > self.swing_high[-1]
#                 if exit1 or exit2 or chop_exit_condition:
#                     print(f"🌙🛑 Closing SHORT Position at {self.data.Close[-1]} (Choppiness: {self.choppiness[-1]:.