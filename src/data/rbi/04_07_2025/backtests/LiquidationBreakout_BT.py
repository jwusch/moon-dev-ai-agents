...
class LiquidationBreakout(Strategy):
    ...
    def init(self):
        # 🌌 Core Indicators
        self.ema = self.I(talib.EMA, self.data.Close, self.kc_ema)
        self.atr_kc = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, self.kc_ema)
        ...
        self.kc_width_avg = self.I(talib.SMA, self.kc_width, 20)
        
        # 🔭 Liquidation Proxies
        self.swing_high = self.I(talib.MAX, self.data.High, self.cluster_window)
        self.swing_low = self.I(talib.MIN, self.data.Low, self.cluster_window)
    ...
    def next(self):
        if self.position:
            ...
            short_break = self.data.Close[-1] < sl_prev