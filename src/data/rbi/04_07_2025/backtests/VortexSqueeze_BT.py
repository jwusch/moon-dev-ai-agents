"""
Moon Dev's VortexSqueeze Strategy
...
#                 if (crossover(self.vi_minus, self.vi_plus)) or (price < sar_value) or (price < self.upper_kc[-1]):
#                     print(f"🌙 CLOSING LONG at {price} (VI:{self.vi_plus[-1]:.2f}|SAR:{sar_value:.2f})")
#                     self.position.close()
#             else:
#                 if (crossover(self.vi_plus, self.vi_minus)) or (price > sar_value) or (price > self.lower_kc[-1]):
#                     print(f"🌙 CLOSING SHORT at {price} (VI:{self.vi_minus[-1]:.2