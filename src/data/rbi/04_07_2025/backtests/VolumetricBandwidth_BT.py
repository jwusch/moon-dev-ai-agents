...
class VolumetricBandwidth(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            long_trigger = (crossover(self.data.Close, self.vwap) and 
            ...
            short_trigger = (crossunder(self.data.Close, self.vwap) and 
    ...
    def calculate_position(self, direction):
        ...
        # Stop Calculation 🛑
        risk_distance = 1.5 * (bb_range / bb_mid) * entry_price
        stop_price = (entry_price - risk_distance) if direction == 'long' else (entry_price + risk_distance)
