...
#         
#         # 5-period RSI
#         self.rsi = self.I(talib.RSI, close, 5, name='RSI_5')
#         
#         # Volume SMA (20-period)
#         volume = self.data.Volume
#         self.volume_avg = self.I(talib.SMA, volume, 20, name='VOLUME_SMA_20')
#         
...
#                     print(f"🚀 LONG ENTRY: Size {size} @ {self.data.Open[-1]} | SL: {sl:.2f} ✨")
#             
...
#                     print(f"🚀 SHORT ENTRY: Size {size} @ {self.data.Open[-1]} | SL: {sl:.2f} ✨")
#         
#         # Exit Conditions
#         if self.position.is_long:
#             rsi_exit = self.rsi[-1] < 70 and self.rsi[-2] >= 70
#             price_exit = (self.data.Close[-1] <= self.kc_upper[-1]) and (self.data.Close[-1] >= self.kc_lower[-1])
