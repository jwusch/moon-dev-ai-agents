...
class VolatilitySqueezeSpike(Strategy):
    ...
    def init(self):
        ...
        self.bbw_low = self.I(lambda x: np.minimum.reduce(x[-10:]), self.bbw, name='BBW_10_LOW')  # Replaced talib.MIN with numpy
        
        # Volume MA
        self.vol_ma = self.I(lambda x: np.convolve(x, np.ones(20)/20, mode='valid'), volume, name='VOL_MA20')  # Pure numpy MA
...
#         print("🌕 MOON DEV STRATEGY ACTIVATED 🌕")
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Volume spike check
            volume_spike = current_volume > self.vol_ma[-1] * 1.5  # Added 50% spike threshold
...
#                         print(f"🌑 MOON DEV SHORT SIGNAL 🌑 | Price: {current_close:.2f} | Size: {size} | SL: {stop_price:.2f}")
        ...
        # 💫 EXIT CONDITIONS 💫
            ...
            keltner_lower = self.ema[-1] - 2*self.at