...
# 🌙 Data preparation with lunar precision
...
class LiquidityGapBreach(Strategy):
    ...
    def moon_long(self):
        ...
        if position_size > 0:
            self.buy(size=position_size, sl=sl, 
                    tag=f" LONG | SL:{sl:.1f}")
            print(f" BLAST OFF! Long {position_size} units at {self.data.Close[-1]}!")
    ...
    def moon_short(self):
        ...
        if position_size > 0:
            ...
            print(f" BLACK HOLE ENGAGED! Short {position_size} units at {self.data.Close[-1]}!")

    def long_exits(self, trade):
        current_low = self.data.Low[-1]
        kc_mid = self.keltner_mid[-1]
