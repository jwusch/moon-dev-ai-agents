"""
Moon Dev's LiquidationDivergence Strategy
...
# Here's the fully debugged code with Moon Dev themed improvements and all technical issues fixed:
# 
# ```python
# # -*- coding: utf-8 -*-
# import pandas as pd
...
# 
# # Load and clean data with Moon Dev data validation ✨
...
#     
#     def init(self):
#         print("🌙 MOON DEV INITIALIZING STRATEGY 🌙")
...
#         print("✨ INDICATORS LOADED: RSI(14), Swing High/Low(20), OI Max(8) ✨")
#         
#     def next(self):
#         if len(self.data.Close) < 50:
#             print("🌑 WAITING FOR ENOUGH DATA (50 candles required) 🌑")
#             return
#             
...
#                 if position_size > 0:
#                     print(f"🌙 MOON DEV LONG ENTRY 🌙 | Price: {self.data.Close[-1]:.2f} | Risk: {risk:.2f} | Size: {position_size}")
...
#                 position_size = int(round((self.equity * self.risk_pct) / risk))
#                 if position_size > 0:
#                     print(f"🌙 MOON DEV SHORT ENTRY 🌙 | Price: {self.data.Close[-1]:.2f} | Risk: {risk:.2f} | Size: {position_size}")
