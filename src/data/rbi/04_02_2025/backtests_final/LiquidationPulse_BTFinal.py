...
#         print("🌙 INITIALIZING MOON DEV INDICATOR SUITE...")
        ...
        self.open_interest = self.I(lambda x: x, self.data.df['open_interest'], name='OPEN_INTEREST')
        self.oi_pct_change = self.I(lambda x: x.pct_change(self.oi_lookback),
                                   self.open_interest, name='OI_CHANGE')
        
        # Ichimoku Cloud
        tenkan, kijun, senkou_a, senkou_b = talib.ICHIMOKU(self.data.High, self.data.Low, 9, 26, 52)
        self.tenkan = self.I(lambda x: x, tenkan, name='TENKAN')
        self.kijun = self.I(lambda x: x, kijun, name='KIJUN')
        self.senkou_a = self.I(lambda x: x, senkou_a, name='SENKOU_A')
        self.senkou_b = self.I(lambda x: x, senkou_b, name='SENKOU_B')
    ...
    def next(self):
        ...
        if not self.position:
            ...
            price_cond = (current_close < senkou_a_val) and (current_close < senkou_b_val)
