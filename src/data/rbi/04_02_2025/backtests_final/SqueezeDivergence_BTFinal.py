...
class SqueezeDivergence(Strategy):
    ...
    def init(self):
        ...
        self.bandwidth = self.I(lambda upper, lower: upper - lower, 
                               self.bb_upper, 
                               self.bb_lower, 
                               name='BB_Width')
        self.sma_bandwidth = self.I(talib.SMA, 
                                  self.bandwidth, 
                                  timeperiod=20, 
                                  name='SMA_BB_Width')
...
#         print("🌙 Moon Dev Indicators Initialized! ✨")
    ...
    def next(self):
        ...
        if (is_squeeze and
            price > bb_upper and
            (self.vwap[-2] < self.vwap_ma[-2] and self.vwap[-1] > self.vwap_ma[-1]) and
            volume_ok):
            ...
            if position_size > 0:
                self.buy(size=position_size,
                        ...
                        tp=price + 2*atr_value)
#                 print(f"🚀 MOON DEV LONG ENTRY! Size: {position_size} | TP:"