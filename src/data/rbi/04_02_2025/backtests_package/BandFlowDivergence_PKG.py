"""
Moon Dev's BandFlowDivergence Strategy
...
#                     self.position.close()
#                     print(f"🌙✨ EXIT LONG: Price {current_close:.2f} crossed below KC Upper {self.kc_upper[-1]:.2f}")
#             elif self.position.is_short:
#                 if current_close > self.kc_lower[-1]:
#                     self.position.close()
#                     print(f"🌙✨ EXIT SHORT: Price {current_close:.2f} crossed above KC Lower {self.kc_lower[-1]:.2f}")
#             
#             # Time-based exit
#             if len(self.data) - self.position.entry_bar >= self