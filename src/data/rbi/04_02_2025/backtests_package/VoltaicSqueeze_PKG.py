...
# Here's the fixed and completed code with proper Moon Dev themed debug prints and ensuring no backtesting.lib imports are used:
# 
# ```python
# import pandas as pd
...
#                 if position_size > 0:
#                     self.buy(size=position_size, sl=self.data.Close[-1] - stop_distance, 
#                             tp=self.data.Close[-1] + 2*stop_distance,
#                             tag={'atr_entry': self.atr[-1]})
#                     print(f"🚀 MOON DEV LONG ENTRY: {self.data.Close[-1]:.2f} | Size: {position_size} | ATR: {self.atr[-1]:.2f}")
#             
#             elif short_trigger:
#                 risk_amount = self.equity * self.risk_percent
#                 stop_distance = 2 *