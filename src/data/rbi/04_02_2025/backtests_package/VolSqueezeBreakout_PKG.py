...
#                 (self.data.Close[-2] < self.keltner_upper[-2] and self.data.Close[-1] > self.keltner_upper[-1])  # Bullish crossover replacement
#             )
#             
...
#                 self.bbw[-1] < self.bbw_percentile[-1] and
#                 self.data.Volume[-1] > self.volume_sma50[-1] and
#                 (self.data.Close[-2] > self.keltner_lower[-2] and self.data.Close[-1] < self.keltner_lower[-1])  # Bearish crossunder replacement
#             )
#             
#             if long_cond:
#                 self.enter_long()
...
#         # Exit logic
#         else:
#             if self.position.is_long and (self.data.Close[-2] > self.ema20[-2] and self.data.Close[-1] < self.ema20[-1]):  # Crossunder replacement
#                 print(f"🌙 Lunar Profit Taken! 🚀 Closing long at {self.data.Close[-1]:.2f}")
#                 self.position.close()
#             
#             elif self.position.is_short and (self.data.Close[-2] < self.ema20[-2] and self.data.Close[-1] > self.ema20[-1]):  # Crossover replacement
#                 print(f