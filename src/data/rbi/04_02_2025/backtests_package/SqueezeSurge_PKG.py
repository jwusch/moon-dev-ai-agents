...
# Here's the fixed code with all backtesting.lib references removed and replaced with proper implementations, along with Moon Dev themed debug prints: 🌙✨
# 
# ```python
# import pandas as pd
# import talib
...
#                 self.highest_high = max(self.highest_high if self.highest_high is not None else 0, self.data.High[-1])
...
#             else:
#                 self.lowest_low = min(self.lowest_low if self.lowest_low is not None else float('inf'), self.data.Low[-1])
