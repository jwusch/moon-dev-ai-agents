...
#             print(f"🌙 Moon Dev Debug: Bar {current_bar} | Close: {self.data.Close[-1]:.2f} | ADX: {self.adx[-1]:.1f}")
#         
#         # Entry Conditions
...
#                 if position_size > 0:
#                     self.buy(size=position_size, tag={'atr_entry': self.atr[-1]})
...
#                 position_size = int(round(risk_amount / stop_distance))
#                 if position_size > 0:
#                     self.sell(size=position_size, tag={'atr_entry': self.atr[-1]})
#                     print(f"🌑 MOON DEV SHORT ENTRY: {self.data.Close[-1]:.2f} | Size: {position_size}")
#         
#         # Exit Conditions
#         for trade in self.t