...
class LiquidationSpikeDivergence(Strategy):
    ...
    def init(self):
        ...
        self.liquidation_mean = self.I(talib.SMA, self.liquidation, 20, name='Liq Mean')
        self.liquidation_std = self.I(talib.STDDEV, self.liquidation, 20, name='Liq STD')
        
        # ===== RSI DIVERGENCE =====
        self.rsi = self.I(talib.RSI, self.data.Close, 14, name='RSI 14')
        
        # ===== CHANDE-KROLL STOP =====
        self.atr = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, 10, name='ATR 10')
        self.max_high = self.I(talib.MAX, self.data.High, 10, name='Max High')
...
#         print("🌠 Moon Dev Indicators Initialized!")
    ...
    def next(self):
        ...
        if not self.position and liq_spike and bullish_divergence:
            ...
            if position_size > 0:
                print(f" MOON SHOT! Long {position_size} @ {entry_price:.2f}")
                self.buy