...
class VolatilitySqueezeTrigger(Strategy):
    ...
    def init(self):
        ...
        self.mid_band = self.I(talib.SMA, self.data.Close, self.bb_period, name='MID_BAND')
        self.std_dev = self.I(talib.STDDEV, self.data.Close, self.bb_period, nbdev=1, name='STD_DEV')
        ...
        roc_ratio = self.I(talib.ROCR, self.data.Close, 1, name='ROC_RATIO')
        log_ret = self.I(np.log, roc_ratio, name='LOG_RET')
        self.hv_20 = self.I(talib.STDDEV, log_ret, self.hv_period, name='HV_20')
        self.hv_avg = self.I(talib.SMA, self.hv_20, self.hv_avg_period, name='HV_AVG')
