...
#             self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, name=['Upper BB', 'Middle BB', 'Lower BB'])
...
#         print("✨ All indicators powered by pure TA-Lib - No backtesting.lib dependencies! ✨")
# 
#     def next(self):
...
#                 self.position.close()
#                 print(f"🌕 LONG Closed at {self.data.Close[-1]:.2f} - Upper BB Touch!")
#             elif self.position.is_short and self.data.Close[-1] < self.lower_bb[-1]:
#                 self.position.close()
#                 print(f"🌑 SHORT Closed at {self.data.Close[-1]:.2f} - Lower BB Touch!")
#             return
# 
