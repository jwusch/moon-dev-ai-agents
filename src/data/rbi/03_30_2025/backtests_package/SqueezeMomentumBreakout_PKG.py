...
class SqueezeMomentumBreakout(Strategy):
    ...
    def next(self):
        ...
        # 🌟 ENTRY LOGIC
        if not self.position and self.trading_enabled:
            ...
            for i in range(3):
                if len(self.data) < self.bb_period + i:
                    continue
                ...
                # CMO cross up - replaced crossover with manual check
                if (self.cmo[-i] > 0) and (self.cmo[-i-1] <= 0):
                    cmo_