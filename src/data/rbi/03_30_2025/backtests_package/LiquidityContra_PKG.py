"""
Moon Dev's LiquidityContra Strategy
...
#         if self.data.Low[-2] > self.swing_low[-2] and self.data.Low[-1] < self.swing_low[-1]:  # Replaced crossunder
...
#         elif self.data.High[-2] < self.swing_high[-2] and self.data.High[-1] > self.swing_high[-1]:  # Replaced crossover
...
#         return all(bw < self.contraction_threshold[i] 
#                   for i, bw in enumerate(self.bandwidth[-self.swing_period:-1], start=