...
#             if self.data.Close[-1] > self.outer_upper[-1] and self.data.Volume[-1] > self.volume_ma[-1] and volatility_ok:
...
#             elif self.data.Close[-1] < self.outer_lower[-1] and self.data.Volume[-1] > self.volume_ma[-1] and volatility_ok:
...
#                 # Retracement exit
#                 if self.data.Close[-1] > self.inner_lower[-1]:
#                     self.position.close()
#                     print(f"🌙✨ Moon Gravity Pull! Closing short at {self.data.Close[-1]:.2f}")
#                 # Time-based exit
#                 elif len(self.data