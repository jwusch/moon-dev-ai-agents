...
#         self.price_swing_high = self.I(talib.MAX, self.data.High, 20, name='Price Swing High 🌙')
#         self.obv_swing_high = self.I(talib.MAX, self.obv, 20, name='OBV Swing High 📈')
#         
#         # Bollinger Bands components
#         self.bb_upper = self.I(lambda c: talib.BBANDS(c, 20, 2, 2, 0)[0], self.data.Close, name='BB Upper 🎢')
#         self.bb_middle = self.I(lambda c: talib.BBANDS(c, 20, 2, 2, 0)[1], self.data.Close, name='BB Middle 📊')
#         self.bb_lower = self.I(lambda c: talib.BBANDS(c, 20, 2, 2, 0)[2], self.data.Close, name='BB Lower 🎢')
#         self.bb_width = self.I(lambda u, l, m: (u - l)/m, self.bb_upper, self.bb_lower, self.bb_middle, name='BB Width 🌊')
#         self.bb_width_min = self.I(talib.MIN, self.bb_width, 20, name='BB Width Min 🔍')
# 
#     def next(self):
