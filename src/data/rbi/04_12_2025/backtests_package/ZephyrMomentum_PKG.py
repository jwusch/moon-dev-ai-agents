"""
Moon Dev's ZephyrMomentum Strategy
...
#         long_cond = all([
#             self.data.Close[-2] < self.vwap[-2] and self.data.Close[-1] > self.vwap[-1],  # Replaced crossover
#             z > 1,
#             macd > 0 and macd > self.macd_hist[-2],
#             self.data.Close[-2] < self.tema[-2] and self.data.Close[-1] > self.tema[-1]  # Replaced crossover
#         ])
#         
#         # Short Entry Comet Tail 🌠
#         short_cond = all([
#             self.vwap[-2] < self.data.Close[-2] and self.vwap[-1] > self.data.Close[-1],  # Replaced crossover
#             z < -1,
#             macd < 0 and macd < self.macd_hist[-2],
#             self.tema[-2] < self.data.Close[-2] and self.tema[-1] > self.data.Close[-1]  # Replaced crossover
#         ])
#         
