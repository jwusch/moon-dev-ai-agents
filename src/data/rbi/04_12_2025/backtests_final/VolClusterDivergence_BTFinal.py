"""
Moon Dev's VolClusterDivergence Strategy
...
# Here's the debugged code with all technical fixes while preserving the original strategy logic:
# 
# ```python
# import pandas as pd
# import talib
...
#         stop_loss_distance = 0.75 * atr_value
#         position_size = int(round((self.equity * risk_pct) / stop_loss_distance))
# 
#         # Entry logic
#         if not self.position and len(self.trades) < 3:
#             if long_cond:
#                 tp_price = close + 1.5 * atr_value
#                 sl_price = close - stop_loss_distance
#                 self.buy(size=position_size, sl=sl_price, tp=tp_price)
#                 print(f"🚀🌕 MOONSHOT LONG! Size: {position_size} | SL: {sl_price:.1f} | TP: {tp_price:.1f}")
#                 
#             elif short_cond:
#                 tp_price = close - 1.5 * atr_value
#                 sl_price = close + stop_loss_distance
#                 self.sell(size=position_size, sl=sl_price, tp=tp_price)
#                 print(f"🌑🛸 DARK SIDE SHORT! Size: {position_size} | SL: {sl_price:.1f} | TP: {tp_price:.1f}")
# 
#         # Exit logic
...
# 
# # Data preparation
# data = pd.read_csv('/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/rbi