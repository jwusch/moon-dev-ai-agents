"""
Moon Dev's VortexChaikinTrail Strategy
...
#         self.cmf = self.I(lambda: talib.CMF(self.data.High, self.data.Low, 
#                          self.data.Close, self.data.Volume, 20), name='CMF')
#         
#         # Parabolic SAR (PSAR)
#         self.psar = self.I(lambda: talib.SAR(self.data.High, self.data.Low, 
#                           acceleration=0.02, maximum=0.2), name='PSAR')
#         
#         # Swing High/Low for backup stops
#         self.swing_high = self.I(lambda: talib.MAX(self.data.High, 20), name='Swing High')
#         self.swing_low = self.I(lambda: talib.MIN(self.data.Low, 20), name='Swing Low')
# 
#     def next(self):
#         self.current_bar += 1
#         
...
#             if (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) and cmf_val > 0.05:
...
#             if (self.vi_minus[-2] < self.vi_plus[-2] and self.vi_minus[-1] > self.vi_plus[-1]) and cmf_val < -0.05:
#                 entry_price = self.data.Close[-1]
...
#                         print(f"🌙🚀 SHORT Entry: {entry_price:.2