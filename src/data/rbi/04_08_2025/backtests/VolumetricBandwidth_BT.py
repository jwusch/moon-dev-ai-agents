...
#         # ======== INDICATORS ======== 🌙
#         # Volume Analysis
#         self.volume_avg = self.I(talib.SMA, self.data.Volume, self.volume_avg_period, name='Volume_MA')
#         
#         # Bollinger Bands
#         def bb_upper(close):
#             upper, _, _ = talib.BBANDS(close, self.bb_period, self.bb_std, self.bb_std)
#             return upper
#         self.bb_upper = self.I(bb_upper, self.data.Close, name='BB_Upper')
#         
#         def bb_middle(close):
#             _, middle, _ = talib.BBANDS(close, self.bb_period, self.bb_std, self.bb_std)
#             return middle
...
#         self.bandwidth = self.I(bandwidth, self.bb_upper, self.I(talib.BBANDS, self.data.Close, self.bb_period, self.bb_std, self.bb_std, matype=0)[2], self.bb_middle, name='Bandwidth')
#         
#         # Bandwidth Median
...
# 
#         # Long Entry
#         if not self.position and volume_signal and bandwidth_signal and price_position:
#             atr_value = self.atr[-1] or 1  # Avoid zero division
#             risk_amount = self.risk_percent * self.equity
#             position_size = int(round(risk_amount / (2 * at