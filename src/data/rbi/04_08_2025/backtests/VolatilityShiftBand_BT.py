"""
Moon Dev's VolatilityShiftBand Strategy
...
# Here's the complete backtesting implementation for the VolatilityShiftBand strategy:
# 
# ```python
# import pandas as pd
# import talib
...
#             # Long entry logic 🌙🚀
#             if (crossover(self.hv_10, self.median_band) and vol_ok and vol_ratio > 0.15):
...
#                     print(f"🌙🚀 VOLATILITY BREAKOUT! Long {size} units @ {self.entry_price:.2f} | SL: {self.stop_loss:.2f}")
#             
#             # Short entry logic 🌙🌑
#             elif (crossover(self.median_band, self.hv_10) and vol_ok and vol_ratio > 0.15):
#                 self.entry_price = self.data.Close[-1]
#                 risk_amount = self.equity * self.trade_risk_pct
#                 risk_per_unit = (self.entry_price + 2*atr) - self.entry_price
#                 size = int(round(risk_amount / risk_per_unit))
#                 if size > 0:
#                     self.sell(size=size)
#                     self.stop_loss = self.entry_price + 2*atr
#                     print(f"🌙🌑 VOLATILITY COLLAPSE! Short {size} units @ {self.entry_price:.