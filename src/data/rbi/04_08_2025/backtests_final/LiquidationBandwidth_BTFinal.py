"""
Moon Dev's LiquidationBandwidth Strategy
...
# Here's the debugged and fixed version of your LiquidationBandwidth strategy with Moon Dev themed debug prints and proper position sizing:
# 
# ```python
# # -*- coding: utf-8 -*-
# import numpy as np
...
#         self.band_width = self.I(lambda: (self.upper - self.lower) / self.middle)
#         self.band_width_low = self.I(talib.MIN, self.band_width, timeperiod=35040)  # 1-year low
#         
#         # Volatility metrics
...
#         self.funding_z = self.I(lambda: (self.data.Close - self.sma20) / self.std20)
#         
#         # Synthetic liquidation clusters (volume spikes + price action)
#         self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)
#         self.vol_spike = self.I(lambda: self.data.Volume > 2 * self.vol_sma)
#         self.price_change = self.I(lambda: self.data.Close.pct_change())
#         
#     def next(self):
...
#                         print(f"🌕 Stop Loss: {stop_loss:.2f} | Take Profit: {price + 2 * atr:.2f}")
#                 
#                 elif long_liq: