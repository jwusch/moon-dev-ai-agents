...
#                 print(f"🚀🌕 MOON DEV LONG TRIGGERED! Entry: {entry_price:.2f}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
...
#                 self.sell(size=position_size, tag="🌙 MOON DEV SHORT ENTRY")
                ...
                stop_loss = entry_price + atr_value
                take_profit = entry_price - 2 * atr_value
#                 print(f"🌑💫 MOON DEV SHORT TRIGGERED! Entry: {entry_price:.2f}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
            ...
            if self.position.is_long:
                if self.data.Close[-1] >= take_profit:
                    self.position.close()
#                     print(f"💰✨ MOON DEV TP HIT! Long closed at {self.data.Close"