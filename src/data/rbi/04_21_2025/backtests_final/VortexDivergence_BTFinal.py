"""
Moon Dev's VortexDivergence Strategy
...
#             if trade.is_long and (self.data.Close[-1] < self.cks_long[-1] or (self.vi_minus[-2] > self.vi_plus[-2] and self.vi_minus[-1] < self.vi_plus[-1])):
#                 trade.close()
#                 print(f"🌙✨💼 Closing LONG at {self.data.Close[-1]:.2f} (SL: {self.cks_long[-1]:.2f})")
#             elif trade.is_short and (self.data.Close[-1] > self.cks_short[-1] or (self.vi_plus[-2] > self.vi_minus[-2] and self.vi_plus[-1] < self.vi_minus[-1])):
#                 trade.close()
#                 print(f"🌙✨💼 Closing SHORT at {self.data.Close[-1]:.2f} (SL: {self.cks_short[-1]:.2f})")
# 
#         if self.position:
#             return
# 
...
# 
#         # Long entry
#         if price_bull_div and (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) and self.data.Close[-1] > self.cks_long[-1]:
