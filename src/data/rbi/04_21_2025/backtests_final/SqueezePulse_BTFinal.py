"""
Moon Dev's SqueezePulse Strategy
...
#                 (self.macd[-2] < self.signal[-2] and self.macd[-1] > self.signal[-1]) and 
#                 squeeze):
...
#                   (self.signal[-2] < self.macd[-2] and self.signal[-1] > self.macd[-1]) and 
#                   squeeze):
#                 risk_amount = self.equity * 0.01
#                 sl = self.bb_upper[-1]
