...
#     def init(self):
#         # Bollinger Bands
#         self.upper_bb = self.I(ta.BBANDS, self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)[0], name='UpperBB')
#         self.middle_bb = self.I(ta.BBANDS, self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)[1], name='MiddleBB')
#         self.lower_bb = self.I(ta.BBANDS, self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)[2], name='LowerBB')
#         
#         # Volatility metrics
...
#                     print(f"🌙✨🚀 MOON DEV LONG SIGNAL DETECTED! Price: {entry_price:.2f}, Size: {size}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
...
#                     size = int(round(risk / risk_per_unit))
#                     take_profit = entry_price - 1.5 * self.bb_width[-1]
#                     print(f"🌙✨🚀 MOON DEV SHORT SIGNAL DETECTED! Price: {entry_price:.2f}, Size: {size}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
